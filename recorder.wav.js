class Recorder {
    constructor(set) {
        this.set = {
            ...
            onProcess: function() {}, // fn(buffers,powerLevel,bufferDuration,bufferSampleRate) buffers=[[Int16,...],...]：缓冲的PCM数据，为从开始录音到现在的所有pcm片段；powerLevel：当前缓冲的音量级别0-100，bufferDuration：已缓冲时长，bufferSampleRate：缓冲使用的采样率（当type支持边录边转码(Worker)时，此采样率和设置的采样率相同，否则不一定相同）
            ...set,
        };
    ...
    static Support = function() {
        ...
        if (!scope.getUserMedia) {
            // console.log('yhc---getUserMedia不支持');
            return false;
        }
    ...
    */
    static SampleData = function(pcmDatas, pcmSampleRate, newSampleRate, prevChunkInfo) {
    ...
    _start = () => {
        ...
        var process = (This.process = (ctx.createScriptProcessor || ctx.createJavaScriptNode).call(
            ctx,
            set.bufferSize,
            1,
            1
    ...
    stop = (True, False, autoClose) => {
        ...
        if (This[set.type + '_complete'] && engineCtx) {
            ...
            duration = Math.round((engineCtx.pcmSize / set.sampleRate) * 1000); // 采用后的数据长度和buffers的长度可能微小的不一致，是采样率连续转换的精度问题
        ...
        duration = Math.round((res.length / set.sampleRate) * 1000);
